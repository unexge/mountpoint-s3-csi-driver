...
func (pm *PodMounter) Mount(ctx context.Context, bucketName string, target string, credentialCtx credentialprovider.ProvideContext, args mountpoint.Args, env envprovider.Environment, fsGroup string) error {
	...
	if !isSourceMountPoint {
		err = pm.mountS3AtSource(ctx, source, pod, podPath, bucketName, env, credEnv, authenticationSource, args)
		if err != nil {
...
func (pm *PodMounter) mountS3AtSource(
	ctx context.Context,
	source string,
	mpPod *corev1.Pod,
	podPath string,
...
func (pm *PodMounter) getS3PodAttachmentWithRetry(ctx context.Context, volumeName string, credentialCtx credentialprovider.ProvideContext, fsGroup string) (*crdv2.MountpointS3PodAttachment, string, error) {
